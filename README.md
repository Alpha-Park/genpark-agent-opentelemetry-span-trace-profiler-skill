# genpark-agent-opentelemetry-span-trace-profiler-skill
...
**genpark-agent-opentelemetry-span-trace-profiler-skill** is a deterministic, zero-dependency Python skill engineered for enterprise AI agent governance, distributed observability, and guardrail enforcement pipelines.

> **Executive Capability**: OpenTelemetry distributed agent span trace annotator & latency profiler (Arize Phoenix)
...
result = client.profile_agent_trace()
print(result)
```
...
```json
{
  "mcpServers": {
    "genpark-agent-opentelemetry-span-trace-profiler-skill": {
      "command": "python",
      "args": ["/path/to/genpark-agent-opentelemetry-span-trace-profiler-skill/mcp_server.py"]
    }
  }
}
```
